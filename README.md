...
**Current status:** Full-stack flows are wired.  
- Flask serves `/api/*` with ingestion + analytics  
...
- `GET /api/ping` → DB connectivity check (`SELECT 1`)
- `POST /api/refresh?pages=1` → ingest trending+top-rated from TMDb into SQLite
- `GET /api/summary` → aggregate counts, avg rating, top genres, top languages
- `GET /api/movies?sort=popularity|vote_average&limit=20&page=1` → list movies
- `GET /api/tv?sort=popularity|vote_average&limit=20&page=1` → list TV
- `GET /api/search?q=QUERY&page=1` → live TMDb search (does not store)
